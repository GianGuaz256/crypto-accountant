...
export default function EthereumWalletDetailPage() {
  const { wallets } = useWallet();
  const params = useParams();
  const address = params.address as string;
  
  // Find the wallet info based on address
  const wallet = wallets.ethereum.find(w => w.address === address);
    ...
    return (
      <div className="max-w-4xl mx-auto p-6">
        <div className="flex items-center mb-8">
          ...
          <ChevronRight className="h-4 w-4 mx-2 text-gray-500 dark:text-gray-400" />
          <Link href="/dashboard/wallets/ethereum" className="text-blue-500 hover:text-blue-600 dark:text-blue-400 dark:hover:text-blue-300">
            Ethereum
          </Link>
        </div>
        ...
        <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm p-6 text-center">
          ...
          <p className="text-gray-600 dark:text-gray-400">
            The Ethereum wallet with address {address} could not be found.
          </p>
          <Link 
            href="/dashboard/wallets/ethereum"
            className="mt-4 inline-block px-4 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700 transition-colors"
          >
            Back to Ethereum Wallets
          </Link>
        </div>
      </div>
    );
    ...
    <div className="p-6 max-w-7xl mx-auto">
      ...
      <header className="mb-8">
        <div className="flex flex-col md:flex-row justify-between md:items-center gap-4">
          <div>
            <h1 className="text-3xl font-bold">Ethereum Wallet: {wallet.name}</h1>
      ...
      <div className="grid md:grid-cols-2 gap-6">
        ...
        <div className="bg-white dark:bg-black p-6 border rounded-lg h-full">
          ...
          <p className="mb-4 text-gray-600 dark:text-gray-400">
            Generate a PDF report of your Ethereum transactions with all labels and descriptions.
          </p>
